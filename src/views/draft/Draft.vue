    ...
    >
      <template v-slot:fallback>
        <v-empty-state
          ...
          text="You haven't any draft e-mails yet. When you do, they'll appear here."
...
export default {
  ...
  async mounted() {
    await this.getDraftMails(this.request.page, this.request.perPage)
  },
  created() {
  ...
  methods: {
    ...
    async getDraftMails(page, perPage) {
      this.loading = true

      try {
        const inboxMails = await this.$store.dispatch(ActionTypes.GET_LIST.DRAFT, {
          page: page,
          perPage: perPage,
    ...
    selectEmailToPreview(emailId) {
      ...
      this.$router.push({ name: 'emailDraft', params: { emailId: emailId }  })
    },
    async loadMore({done}) {
        await this.getDraftMails(this.request.page + 1, this.request.perPage)
        done()
    }
  }
}
